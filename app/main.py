...
def update_todo(todo_id: int, toDoItem: schema.ToDoItemUpdate, db: Session = Depends(get_db)):
    return service.update_toDoItem(db, todo_id, toDoItem)

@app.delete("/todos/{todo_id}")
def delete_todo(todo_id: int, db: Session = Depends(get_db)):
    return service.delete_toDoItem(db, todo_id)
